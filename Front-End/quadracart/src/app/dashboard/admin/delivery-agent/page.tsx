...
export default function CustomerPage() {
  ...
  const handleDeleteCustomer = async (id: number) => {
    ...
    if (confirmDelete) {
      try {
        await axios.delete(`http://localhost:3000/delivery-agent/${id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setDeliveryAgents(deliveryAgents.filter(deliveryAgents => deliveryAgents.id !== id));
      } catch (err) {
        handleError(err);
      }
    }
  ...
  const filteredDeliveryAgents = deliveryAgents.filter(deliveryAgent => {
    return (
      deliveryAgent.id.toString().includes(searchTerm) || // Convert ID to string for comparison
      deliveryAgent.deliveryAgentname.toLowerCase().includes(searchTerm.toLowerCase()) ||
      deliveryAgent.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (deliveryAgent.phonenumber && deliveryAgent.phonenumber.includes(searchTerm)) // Check if phonenumber exists
    );
  });
  ...
  return (
    ...
    <div className="p-8">
      <h1 className="text-3xl font-semibold mb-6">Delivery-Agents</h1>
      <input
            type="text"
      ...
      <div className="space-y-4">
        {filteredDeliveryAgents.map(deliveryAgents => (
          <div key={deliveryAgents.id} className="p-4 border rounded-lg bg-white">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div><span className="font-semibold">ID:</span> {deliveryAgents.id}</div>
              <div><span className="font-semibold">Agent Name:</span> {deliveryAgents.deliveryAgentname}</div>
              <div><span className="font-semibold">Email:</span> {deliveryAgents.email}</div>
              <div><span className="font-semibold">Phone:</span> {deliveryAgents.phonenumber}</div>
              <div><span className="font-semibold">Address:</span> {deliveryAgents.address}</div>
              <div><span className="font-semibold">Vehicle Type:</span> {deliveryAgents.vehicleType}</div>

            </div>
            <Button 
              onClick={() => handleDeleteCustomer(deliveryAgents.id)}
