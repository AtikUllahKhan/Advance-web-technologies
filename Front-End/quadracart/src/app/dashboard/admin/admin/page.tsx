...
export default function AdminPage() {
  ...
  const filteredAdmins = admins.filter(admin => {
    return (
      admin.id.toString().includes(searchTerm) || // Convert ID to string for comparison
      admin.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      admin.email.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (admin.phonenumber && admin.phonenumber.includes(searchTerm)) // Check if phonenumber exists
    );
  });
        ...
        <div className="p-8">
          ...
          <div className="space-y-4">
            {filteredAdmins.map(admin => (
              <div key={admin.id} className="p-4 border rounded-lg bg-white">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div><span className="font-semibold">ID:</span> {admin.id}</div>
