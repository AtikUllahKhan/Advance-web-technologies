...
export default function DeliveryAgentRegistration() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Validate inputs
    if (!formData.deliveryAgentname) newError.deliveryAgentname = "Please enter your name.";
    ...
    if (!formData.vehicleType) newError.vehicleType = "Please enter your vehicle type.";
    ...
    try {
      const response = await axios.post("http://localhost:3000/delivery-agent/register", formData);
      setSuccess(response.data.message || "Delivery Agent registered successfully!");

      router.push(`/otp/delivery-agent?email=${encodeURIComponent(formData.email)}`);
    } catch (err: unknown) {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-cover bg-center">
      <div className="relative w-full max-w-lg p-8 bg-[#b9f5cf] rounded-xl shadow-lg">
        {/* Close Button */}
        <button
        ...
        <CardContent>
          <h1 className="text-3xl font-semibold text-center mb-6 text-gray-800">Delivery Agent Registration</h1>
          {success && <div className="text-green-500 mb-4">{success}</div>}
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="deliveryAgentname" className="block text-sm font-medium mb-1">Agent Name</label>
              <input
                type="text"
                ...
                required
              />
              {error.deliveryAgentname && <div className="text-red-500 text-sm mt-1">{error.deliveryAgentname}</div>}
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                required
              />
              {error.vehicleType && <div className="text-red-500 text-sm mt-1">{error.vehicleType}</div>}
            </div>
            <div>
